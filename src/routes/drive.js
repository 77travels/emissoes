...
router.get('/status', requireRole('master', 'admin'), async (req, res, next) => {
  try {
    res.json({ configured: drive.isConfigured(), connected: await drive.isConnected() });
  } catch (e) { next(e); }
});
...
router.post('/disconnect', requireRole('master', 'admin'), async (req, res, next) => {
  try {
...
router.post('/sync', requireRole('master', 'admin'), async (req, res, next) => {
  try {
    if (!(await drive.isConnected())) return res.status(400).json({ error: 'Google Drive não conectado' });
